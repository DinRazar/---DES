...
class MyDES:
    ...
    def _iteration(self, block: str, key: str, is_decode: bool) -> str:
        ...
        for i in range(16):
            # Разделить на два субблока
            left, right = block[0: 32], block[32: 64]
             # Используйте правую часть этого раунда исходного видения как левую часть следующего раунда
            next_left = right
             # f функция
...
if __name__ == '__main__':    
    md = MyDES()
    # Считывание файла с начальным текстом
    with open('decrypted_text.txt', 'r', encoding='utf-8') as f1:
        decrypted_text = f1.readline()
    # Считывание файла с ключом
    with open('key.txt', 'r', encoding='utf-8') as f2:
        key = f2.readline()
    # Применение функции шифрования
    encrypted_text = md.encode(decrypted_text, key)
    # Запеись зашифрованного текста в текстовый файл
    with open('encrypted_text.txt', 'a', encoding='utf-8') as f3:
        data = encrypted_text
        f3.write(data)
    
    print("Зашифрованные данные DES:" + encrypted_text)
    print(f"Расшифрованные данные:" + md.decode(encrypted_text, key))
    
    # key = "D15"
    # print("Зашифрованные данные DES:" + md.encode("Dmitriy", key))
    # print(f"Расшифрованные данные:" + md.decode("0x841321c8a27dceb8", key))
